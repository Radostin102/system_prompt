...
- **Python:**
    - Always use type hints (never use the `typing` module, use built-in types instead, e.g., `list[str]` instead of `List[str]`)
    - Always use decorators (e.g., `@staticmethod`)
- **Roblox Luau:**
    - Always use `task.wait` instead of `wait`
    - Always write `for _, child in lighting:GetChildren() do` instead of `for _, child in ipairs(lighting:GetChildren()) do`
